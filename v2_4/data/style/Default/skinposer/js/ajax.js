...
function SetGenderSkin(id, current) {
    ...
    var event = function(response) {
        ...
        if (response['code'] == 0) {
            GetById('skin-change-gender-' + id).onclick = function() {
                SetGenderSkin(id, set);
                return false
            }
            GetById('skin-gender-' + id).src = g_img_urls[set]
        }
    }
...
function DeleteSkin(id) {
    ...
    var event = function(response) {
        ...
        if (response['code'] == 0) {
            ...
            var trash = GetById('skin-' + id)

            fadeElement(trash, function() {
                if (trash == null)
...
function uploadSkin() {
    ...
    if (GetById('uplskin').value.length < 1)
        return false
