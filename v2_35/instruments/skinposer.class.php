...
private $user_id;
	...
	if ( !empty($user) and $user->Exist() ) $user_id = $user->id(); 
	else $user_id = 0;
...
private $answer;
	...
	if ( !$skins_count ) return $this->ShowById(0); //
	...
	global $user;		
		...
		if (!$skins_count) $html_skin_list = $this->ShowPage('skin_empty.html');
			...
			if ( !mysql_num_rows( $result )) $html_skin_list = $this->ShowPage('skin_empty.html');
